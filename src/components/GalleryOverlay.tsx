...
const GalleryOverlay = () => {
  const { isGalleryOpen, closeGallery } = useUI();
  const [viewMode, setViewMode] = useState<ViewMode>("grid");
  const [selectedWork, setSelectedWork] = useState<Artwork | null>(null);
  const lightboxRef = useRef<HTMLDivElement>(null);
  const navigate = useNavigate();

  // Close lightbox on Escape, or close gallery if lightbox is closed
  useEffect(() => {
  ...
  }, [selectedWork, isGalleryOpen, closeGallery]);
  ...
  const handleGoToWork = (id: string) => {
    setSelectedWork(null);
    closeGallery();
    navigate(`/works/${id}`);
  };
        ...
        >
          <div className="pt-28 md:pt-36 pb-24 px-6 md:px-12 lg:px-20 relative min-h-screen max-w-7xl mx-auto">
            ...
            <button
              ...
              className="fixed top-8 right-8 md:top-12 md:right-12 z-[101] text-muted-foreground hover:text-foreground transition-colors mix-blend-difference"
            ...
            <FadeInView>
              <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-6">
                <div>
                  <h2 className="font-heading text-4xl md:text-5xl tracking-[0.05em] text-inherit opacity-85">
                    Gallery View
                  </h2>
                ...
                {/* View toggle */}
                <div className="flex items-center gap-1 self-start md:self-auto mr-12 md:mr-0">
                  <button
                ...
                >
                  <div className="gallery-lightbox__image-section">
                    <ProtectedImage
                      ...
                      className="gallery-lightbox__image cursor-pointer"
                      onClick={(e) => {
                        e.stopPropagation();
                        handleGoToWork(selectedWork.id);
                      }}
                    />
                  </div>
                  ...
                  <div className="gallery-lightbox__details">
                    ...
                    <button
                      onClick={() => handleGoToWork(selectedWork.id)}
                      className="gallery-lightbox__link inline-flex items-center gap-2 mt-4"
                    >
                      Full Experience →
                    </button>
                  </div>
